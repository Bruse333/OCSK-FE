...
- 路由守卫：未登录跳 `/login`；`/system/statistics` 与 `/system/users` 仅 `privilege === 3` 可访问。
...
| `1` | 仅检索 |
| `2` | 检索 + 上传/编辑 |
| `3` | 全部权限（可删除、新增用户、查看系统管理-数据统计/用户管理） |
...
| `/users` | GET | 查询用户列表 | UserManagement |
| `/users` | POST | 新增用户（需 privilege=3） | UserManagement |
| `/ships` | GET | 获取船型列表 | Retrieval、AddRecord |
...
| `/upload` | POST | 文件/图片上传 | Retrieval、AddRecord |
...
- ~~原首页的"新增用户"入口已删除~~，统一迁移至系统管理-用户管理。
